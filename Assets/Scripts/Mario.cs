...
public class Mario : MonoBehaviour {
	...
	//private float waitBetweenFire = 0.1f;
	...
	void FixedUpdate () 
	{
        ...
        if (moveDirectionX != 0 && t_LevelManager.marioSize == 0)
            m_Animator.SetBool("isRunningSmall", true);
        if (moveDirectionX != 0 && t_LevelManager.marioSize == 1)
            m_Animator.SetBool("isRunningBig", true);
        if (moveDirectionX != 0 && t_LevelManager.marioSize == 2)
            m_Animator.SetBool("isRunningFire", true);

		
		float _joystickVerticalOffset = MoveJoystick.Vertical;
		if (ChoosePlatform._pcDevice == false)
			if (_joystickVerticalOffset <= -0.5f)
				isCrouching = true;
    }
	...
	public void Jump()
    {
		...
		{
			isJumping = true;
			m_Rigidbody2D.velocity = Vector2.up * _jumpForce;
            SetJumpParams();
	...
	{
		if (ChoosePlatform._pcDevice == true)
		{
		...
		if (t_LevelManager.marioSize == 2 && ChoosePlatform._pcDevice == false)
			FireButton.SetActive(true);
        else if (t_LevelManager.marioSize == 1 || t_LevelManager.marioSize == 0)
            FireButton.SetActive(false);
    }
    ...
    {
        currentSpeedX = 0;
        m_Animator.SetBool("isRunningSmall", false);
        m_Animator.SetBool("isRunningBig", false);
        m_Animator.SetBool("isRunningFire", false);
    }
	*/
    ...
    void Update() 
	{
		...
		if (!inputFreezed) 
		{
			if (ChoosePlatform._pcDevice == true)
				...
				faceDirectionX = MoveJoystick.Horizontal;
...
//	Vector2 deadDownVelocity = new Vector2 (0, -15f);
	...
	void OnCollisionEnter2D(Collision2D other) 
	{
		...
		if (other.gameObject.tag.Contains("Enemy")) // TODO: koopa shell static does no damage
		{
			...
			if (t_LevelManager.isInvincible() == false) 
			{
